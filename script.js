// Step 3: Add a Radius Slider.
// // Task: Implement a slider that lets users change the radius (in km) within which the parks are shown.

// Step 4: Add Custom Controls.
// Task: Add custom zoom buttons, a reset button to re-center the map, and a toggle button to switch between map and satellite views.

// Step 5: Implement User Geolocation.
// Task: Allow users to find parks near their current location (use browser geolocation to center the map on their location).

// Step 6: Add Custom Info Windows with Photos
// Task: Modify the info windows so they show additional information, including photos and user reveiws and ratings when available, about each park.

// Step 7: Cluster Markers for Better Visualization
// Task: Use marker clustering to group park markers when zoomed out.

// Step 8: Add Directions to Selected Parks.
// Task: Add a button to each info window that provides walking directions from the user’s current location to the selected park.

// Step 9: Pass the Lighthouse PWA tests and submit link on Google Classroom.
